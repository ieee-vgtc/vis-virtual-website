...
def bucket_info():
    ...
    return dict(
        (o.key, {"Key": o.key, "Size": o.size, "ETag": o.e_tag})
...
def set_mimetype(key, mime):
    bucket.copy(
        {"Bucket": target_bucket_name, "Key": key}, key, ExtraArgs={"ContentType": mime}
    )
...
def delete_objects(objs):
    ...
    print(
        'bucket.delete_objects(Delete={"Objects":%s, "Quiet":False})' % repr(obj_list)
    )
...
def put_objects(objs):
    for obj in objs:
        mime_type = utils.my_guess_mimetype(obj)
        f = open(obj, "rb")
        print(
            "bucket.put_object(Key=%s, Body=f, ContentType=%s)"
            % (repr(obj), repr(mime_type))
        )
        bucket.put_object(
            Key=obj, Body=f, ContentType=mime_type, Metadata={"git_sha": branch_sha}
        )
...
def run_cmd_get_lines(*cmd):
    ...
    return subprocess.run(
        list(cmd), check=True, stdout=subprocess.PIPE, universal_newlines=True
    ).stdout.split("\n")
...
def check_if_git_is_clean():
    ...
    if branch != git_branch_name:
        raise Exception(
            "Need git to be in the correct branch.\nExpected to be in branch '%s', but it seems we're in branch '%s' instead."
            % (git_branch_name, branch)
        )

    l = list(
        filter(lambda s: s != "", run_cmd_get_lines("git", "status", "--porcelain"))
    )
    if len(l) != 0:
        raise Exception(
            "Expected git working tree to be clean, but it appears not to be."
        )
    ...
    l = list(
        filter(
            lambda s: remote_ref in s, run_cmd_get_lines("git", "ls-remote", git_remote)
        )
    )
    if len(l) != 1:
    ...
    l = list(
        filter(lambda s: s.startswith("*"), run_cmd_get_lines("git", "branch", "-v"))
    )
    if len(l) != 1:
    ...
    if not remote_sha.startswith(local_branch_sha):
        raise Exception(
            "remote branch sha (%s) doesn't match local branch sha: (%s) "
            % (remote_sha, local_branch_sha)
        )
...
diff = utils.diff_local_remote_buckets(local_info(), bucket_info())
